...
import { ArrowLeft, Mail, Phone, MapPin, Calendar, Shield, Ruler, Weight, Heart, Activity, Star, Trophy, CreditCard as Edit3, MessageSquare, Clock, Target, Zap, BookOpen, CheckCircle2, PlayCircle, PauseCircle, Award, ChevronRight, Users, GraduationCap, BarChart2 } from 'lucide-react';
import { MatchsTab } from '../components/MatchsTab';
import { EvaluationsTab } from '../components/EvaluationsTab';
import { InformationsTab } from '../components/InformationsTab';
import { StatistiquesTab } from '../components/statistiques/StatistiquesTab';
...
const playersData: Record<string, {
  ...
  '1': {
    ...
    email: 'abdelmalek@gmail.com', phone: '+216 55 123 456',
...
const questionnaires: Questionnaire[] = [
  ...
  { id: 4, nom: 'Récupération post-match',coach: 'Coach Slim', envoyes: 5,  startDate: '01/10/2024', reponses: 3,  statut: 'actif' },
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <h2 className="text-xl font-semibold text-default-font">Questionnaires</h2>
          <span className="text-xs text-subtext-color bg-neutral-100 px-2.5 py-1 rounded-full font-medium">
        ...
        {questionnaires.map((q) => {
          ...
          return (
            <button
              ...
              className="w-full text-left bg-neutral-50 rounded-lg border border-neutral-border hover:border-brand-600 hover:bg-neutral-100 transition-all duration-200 p-5"
            >
              <div className="flex items-center justify-between gap-6">
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className="text-base font-semibold text-default-font truncate">
                        {q.nom}
                      </h3>
                      <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium flex-shrink-0 ${statutConfig[q.statut].badge}`}>
                        {statutConfig[q.statut].label}
                      </span>
                    </div>

                    {/* Meta Info */}
                    <div className="flex items-center gap-4 text-xs text-subtext-color">
                ...
                <div className="flex items-center gap-6 flex-shrink-0">
                  ...
                  <div className="text-center min-w-[60px]">
                    <div className="text-lg font-semibold text-default-font mb-0.5">
                      {q.envoyes}
                    </div>
                  ...
                  {/* Responses */}
                  <div className="text-center min-w-[60px]">
                    <div className="text-lg font-semibold text-success-700 mb-0.5">
                      {q.reponses}
                    </div>
                    <div className="text-xs text-subtext-color">
                      Réponses
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    <div 
                      ...
                      style={{ 
                        background: `conic-gradient(rgb(85, 180, 103) ${tauxReponse * 3.6}deg, rgb(48, 48, 48) 0deg)` 
                      }}
                    >
                      <div className="w-9 h-9 rounded-full bg-neutral-50 flex items-center justify-center">
...
type FormationPresence = 'present' | 'absent' | 'justifie' | 'retard';

interface FormationSession {
  date: string;
  titre: string;
  duree: string;
  presence: FormationPresence;
}
...
const formationPresenceConfig: Record<FormationPresence, { label: string; badge: string }> = {
  present:  { label: 'Present',   badge: 'bg-success-50 text-success-600 border border-success-200' },
  absent:   { label: 'Absent',    badge: 'bg-error-50 text-error-600 border border-error-200'       },
  justifie: { label: 'Justifie',  badge: 'bg-warning-50 text-warning-600 border border-warning-200' },
  retard:   { label: 'Retard',    badge: 'bg-brand-50 text-brand-600 border border-brand-200'       },
};
...
const formations: Formation[] = [
  {
    ...
    categorie: 'Langue',
    description: 'Vocabulaire sportif, entretiens media, communication avec les coequipiers etrangers.',
    ...
    certifie: false,
    sessions: [
      { date: '10/09/2024', titre: 'Introduction & vocabulaire de base',             duree: '2h', presence: 'present'  },
      { date: '17/09/2024', titre: 'Vocabulaire terrain — positions & consignes',    duree: '2h', presence: 'present'  },
      { date: '24/09/2024', titre: 'Communication vestiaire & expressions courantes',duree: '2h', presence: 'retard'   },
      { date: '01/10/2024', titre: 'Simulation interview media',                     duree: '2h', presence: 'present'  },
      { date: '08/10/2024', titre: 'Entretiens presse — exercice pratique',          duree: '2h', presence: 'absent'   },
      { date: '15/10/2024', titre: 'Expression orale — niveau intermediaire',        duree: '2h', presence: 'present'  },
      { date: '22/10/2024', titre: 'Comprehension orale — accents & dialectes',     duree: '2h', presence: 'present'  },
      { date: '29/10/2024', titre: 'Redaction professionnelle basique',             duree: '2h', presence: 'justifie' },
      { date: '05/11/2024', titre: 'Vocabulaire contractuel & agents',              duree: '2h', presence: 'present'  },
      { date: '12/11/2024', titre: 'Jeu de role — negociation en anglais',          duree: '2h', presence: 'present'  },
    ],
  },
  {
    id: 2,
    titre: 'Preparation mentale & gestion du stress',
    categorie: 'Mental',
    description: 'Techniques de visualisation, controle des emotions, concentration et confiance en soi avant les matchs.',
    ...
    certifie: true,
    sessions: [
      { date: '01/10/2024', titre: 'Psychologie sportive — fondamentaux',           duree: '2h', presence: 'present' },
      { date: '08/10/2024', titre: 'Techniques de visualisation',                   duree: '2h', presence: 'present' },
      { date: '15/10/2024', titre: 'Routines pre-match personnalisees',             duree: '2h', presence: 'present' },
      { date: '22/10/2024', titre: 'Gestion du stress — respiration & ancrage',     duree: '2h', presence: 'retard'  },
      { date: '29/10/2024', titre: 'Gestion de la pression en competition',         duree: '2h', presence: 'present' },
      { date: '05/11/2024', titre: 'Confiance en soi — exercices pratiques',        duree: '2h', presence: 'present' },
      { date: '12/11/2024', titre: 'Leadership sur le terrain',                      duree: '2h', presence: 'present' },
      { date: '19/11/2024', titre: 'Cohesion de groupe & communication',            duree: '2h', presence: 'present' },
      { date: '26/11/2024', titre: 'Gestion des echecs & resilience',               duree: '2h', presence: 'justifie'},
      { date: '03/12/2024', titre: 'Atelier focus & concentration',                 duree: '2h', presence: 'present' },
      { date: '10/12/2024', titre: 'Revision & synthese',                           duree: '2h', presence: 'present' },
      { date: '17/12/2024', titre: 'Evaluation finale',                              duree: '2h', presence: 'present' },
    ],
  },
  {
    id: 3,
    ...
    categorie: 'Nutrition',
    description: 'Alimentation pre et post-match, hydratation, complements alimentaires autorises, gestion du poids.',
    ...
    certifie: false,
    sessions: [
      { date: '15/11/2024', titre: 'Les macronutriments et l\'energie',             duree: '2h', presence: 'present'  },
      { date: '22/11/2024', titre: 'Hydratation & performance',                     duree: '2h', presence: 'present'  },
      { date: '29/11/2024', titre: 'Nutrition pre-competition',                      duree: '2h', presence: 'absent'   },
      { date: '06/12/2024', titre: 'Recuperation nutritionnelle post-match',         duree: '2h', presence: 'present'  },
      { date: '13/12/2024', titre: 'Complements alimentaires — ce qui est autorise',duree: '2h', presence: 'retard'   },
      { date: '20/12/2024', titre: 'Reglementation antidopage',                     duree: '2h', presence: 'present'  },
    ],
  },
  {
    id: 4,
    titre: 'Regles du jeu & fair-play',
    categorie: 'Réglementaire',
    description: 'Lois du jeu FIFA, conduite sportive, respect de l\'arbitre, reglement disciplinaire du club.',
    ...
    certifie: true,
    sessions: [
      { date: '01/09/2024', titre: 'Lois du jeu FIFA (edition 2024)',        duree: '2h', presence: 'present' },
      { date: '08/09/2024', titre: 'Ethique sportive & fair-play',            duree: '2h', presence: 'present' },
      { date: '15/09/2024', titre: 'Reglement interieur du club',             duree: '2h', presence: 'present' },
      { date: '22/09/2024', titre: 'Quiz de validation',                      duree: '2h', presence: 'present' },
    ],
  },
  {
    id: 5,
    titre: 'Leadership & esprit d\'equipe',
    categorie: 'Leadership',
    description: 'Developper son role au sein du groupe, communication positive, gestion des conflits et capitanat.',
  ...
  {
    id: 6,
    titre: 'Analyse video & comprehension tactique',
    categorie: 'Tactique',
    description: 'Lecture du jeu, analyse des schemas adverses, prise de decision rapide et positionnement.',
    ...
    certifie: false,
    sessions: [
      { date: '01/10/2024', titre: 'Introduction a l\'analyse video',          duree: '2h', presence: 'present' },
      { date: '08/10/2024', titre: 'Schemas tactiques offensifs — 4-3-3',     duree: '2h', presence: 'present' },
      { date: '15/10/2024', titre: 'Schemas tactiques offensifs — 4-4-2',     duree: '2h', presence: 'absent'  },
      { date: '22/10/2024', titre: 'Schemas tactiques defensifs — pressing',  duree: '2h', presence: 'present' },
      { date: '29/10/2024', titre: 'Analyse d\'un adversaire — cas pratique', duree: '2h', presence: 'retard'  },
    ],
  },
];
...
function FormationDetail({ f, onBack }: { f: Formation; onBack: () => void }) {
  ...
  const presentCount = f.sessions.filter(s => s.presence === 'present').length;
  const absentCount = f.sessions.filter(s => s.presence === 'absent').length;
  const justifieCount = f.sessions.filter(s => s.presence === 'justifie').length;
  const retardCount = f.sessions.filter(s => s.presence === 'retard').length;
  const tauxPresence = totalSessions > 0 ? Math.round((presentCount / totalSessions) * 100) : 0;

  const detailKpis = [
    { label: 'Sessions',  value: totalSessions, color: 'text-default-font',  iconBg: 'bg-brand-50',   iconColor: 'text-brand-600',   iconType: 'total' as const   },
    { label: 'Present',   value: presentCount,  color: 'text-success-600',   iconBg: 'bg-success-50', iconColor: 'text-success-600', iconType: 'present' as const },
    { label: 'Absent',    value: absentCount,   color: 'text-error-600',     iconBg: 'bg-error-50',   iconColor: 'text-error-600',   iconType: 'absent' as const  },
    { label: 'Justifie',  value: justifieCount, color: 'text-warning-600',   iconBg: 'bg-warning-50', iconColor: 'text-warning-600', iconType: 'justifie' as const},
    { label: 'Retard',    value: retardCount,   color: 'text-brand-600',     iconBg: 'bg-brand-50',   iconColor: 'text-brand-600',   iconType: 'retard' as const  },
  ];
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-5 mb-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-neutral-200">
          {[
            ...
            { icon: <Calendar className="w-3.5 h-3.5 text-brand-600" />,     label: 'Debut',        value: f.dateDebut  },
            { icon: <Calendar className="w-3.5 h-3.5 text-brand-600" />,     label: 'Fin prevue',   value: f.dateFin    },
            { icon: <Clock className="w-3.5 h-3.5 text-brand-600" />,        label: 'Duree totale', value: f.dureeTotal },
      ...
      {totalSessions > 0 && (
        <>
          <div className="grid grid-cols-5 border border-neutral-200 rounded-lg mb-5 divide-x divide-neutral-200 bg-neutral-50">
            {detailKpis.map(({ label, value, color, iconBg, iconColor, iconType }) => (
          ...
          <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-4 mb-5">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-subtext-color">Taux de presence</span>
              <span className={`text-sm font-semibold ${tauxPresence >= 75 ? 'text-success-600' : tauxPresence >= 50 ? 'text-warning-600' : 'text-error-600'}`}>
                {tauxPresence}%
              </span>
            </div>
            <div className="h-2.5 bg-neutral-100 rounded-full overflow-hidden mb-2">
              <div
                className={`h-full rounded-full transition-all ${tauxPresence >= 75 ? 'bg-success-600' : tauxPresence >= 50 ? 'bg-warning-600' : 'bg-error-600'}`}
                style={{ width: `${tauxPresence}%` }}
              />
            </div>
            <div className="flex flex-wrap gap-4 mt-3">
              {[
                { label: 'Present',  pct: totalSessions > 0 ? Math.round((presentCount / totalSessions) * 100) : 0,  dot: 'bg-success-600' },
                { label: 'Absent',   pct: totalSessions > 0 ? Math.round((absentCount / totalSessions) * 100) : 0,   dot: 'bg-error-600'   },
                { label: 'Justifie', pct: totalSessions > 0 ? Math.round((justifieCount / totalSessions) * 100) : 0, dot: 'bg-warning-600' },
                { label: 'Retard',   pct: totalSessions > 0 ? Math.round((retardCount / totalSessions) * 100) : 0,   dot: 'bg-brand-600'   },
              ].map(({ label, pct, dot }) => (
                <div key={label} className="flex items-center gap-1.5">
                  <div className={`w-2 h-2 rounded-full flex-shrink-0 ${dot}`} />
                  <span className="text-xs text-subtext-color">{label} <span className="font-medium text-default-font">{pct}%</span></span>
      ...
      <div className="flex items-center gap-2 mb-4">
        <h3 className="text-xs font-semibold text-subtext-color uppercase tracking-wide">Sessions</h3>
        <span className="text-xs text-subtext-color bg-neutral-100 px-2 py-0.5 rounded-full">{totalSessions} sessions</span>
      </div>

      {totalSessions === 0 ? (
        <div className="py-10 text-center text-subtext-color text-sm border border-neutral-200 rounded-lg bg-neutral-50">
          Aucune session programmee pour cette formation.
        </div>
      ) : (
        <div className="border border-neutral-200 rounded-lg overflow-hidden">
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-neutral-100 border-b border-neutral-200">
                <th className="text-left px-4 py-3 text-xs font-medium text-subtext-color w-24">Date</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-subtext-color">Session</th>
                <th className="text-center px-4 py-3 text-xs font-medium text-subtext-color w-20">Duree</th>
                <th className="text-center px-4 py-3 text-xs font-medium text-subtext-color w-28">Presence</th>
              </tr>
            </thead>
            <tbody className="bg-neutral-50 divide-y divide-neutral-200">
              {f.sessions.map((s, i) => (
                <tr key={i} className="hover:bg-neutral-100 transition-colors">
                  <td className="px-4 py-3 text-xs text-subtext-color tabular-nums whitespace-nowrap">{s.date}</td>
                  <td className="px-4 py-3"><span className="text-sm text-default-font">{s.titre}</span></td>
                  <td className="px-4 py-3 text-center text-xs text-subtext-color tabular-nums">{s.duree}</td>
                  <td className="px-4 py-3 text-center">
                    <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium ${formationPresenceConfig[s.presence].badge}`}>
                      {formationPresenceConfig[s.presence].label}
                    </span>
                  </td>
                </tr>
              ))}
...
function FormationTab() {
  ...
  const globalTotal      = allSessions.length;
  const globalPresent    = allSessions.filter(s => s.presence === 'present').length;
  const globalAbsent     = allSessions.filter(s => s.presence === 'absent').length;
  const globalRetard     = allSessions.filter(s => s.presence === 'retard').length;
  const globalTaux       = globalTotal > 0 ? Math.round((globalPresent / globalTotal) * 100) : 0;

  const kpis = [
    { label: 'Formations',     value: totalFormations, icon: <BookOpen className="w-4 h-4 text-brand-600" />,      cls: 'text-default-font'  },
    { label: 'Terminees',      value: terminees,       icon: <CheckCircle2 className="w-4 h-4 text-success-600" />,cls: 'text-success-600'   },
    { label: 'En cours',       value: enCours,         icon: <PlayCircle className="w-4 h-4 text-brand-600" />,    cls: 'text-brand-600'     },
    { label: 'Non commencees', value: nonCommence,     icon: <PauseCircle className="w-4 h-4 text-subtext-color" />,cls: 'text-subtext-color'},
    { label: 'Certifications', value: certifications,  icon: <Award className="w-4 h-4 text-warning-600" />,       cls: 'text-warning-600'   },
    { label: 'Taux presence',  value: `${globalTaux}%`,icon: <Target className="w-4 h-4 text-success-600" />,      cls: globalTaux >= 75 ? 'text-success-600' : globalTaux >= 50 ? 'text-warning-600' : 'text-error-600' },
  ];
  ...
  const filterOptions: { value: FormationFilter; label: string }[] = [
    ...
    { value: 'non_commence',   label: 'Non commencees' },
  ...
  return (
    <div className="p-6">
      ...
      <div className="bg-neutral-50 border border-neutral-200 rounded-lg p-4 mb-6">
        <div className="flex items-center justify-between mb-2">
          <span className="text-xs font-medium text-subtext-color">Presence globale aux formations</span>
          <span className={`text-sm font-semibold ${globalTaux >= 75 ? 'text-success-600' : globalTaux >= 50 ? 'text-warning-600' : 'text-error-600'}`}>
            {globalPresent}/{globalTotal} sessions
          </span>
        </div>
        <div className="h-2 bg-neutral-100 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all ${globalTaux >= 75 ? 'bg-success-600' : globalTaux >= 50 ? 'bg-warning-600' : 'bg-error-600'}`} style={{ width: `${globalTaux}%` }} />
        </div>
        <div className="flex flex-wrap gap-4 mt-3">
          {[
            { label: 'Present',  val: globalPresent, dot: 'bg-success-600' },
            { label: 'Absent',   val: globalAbsent,  dot: 'bg-error-600'   },
            { label: 'Retard',   val: globalRetard,  dot: 'bg-brand-600'   },
          ].map(({ label, val, dot }) => (
            <div key={label} className="flex items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full flex-shrink-0 ${dot}`} />
              <span className="text-xs text-subtext-color">{label} <span className="font-medium text-default-font">{val}</span></span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {filtered.map((f) => {
          const statConf = formationStatutConfig[f.statut];
          const catConf  = categorieConfig[f.categorie];
          const fTotal   = f.sessions.length;
          const fPresent = f.sessions.filter(s => s.presence === 'present').length;
          const fAbsent  = f.sessions.filter(s => s.presence === 'absent').length;
          const fRetard  = f.sessions.filter(s => s.presence === 'retard').length;
          const fTaux    = fTotal > 0 ? Math.round((fPresent / fTotal) * 100) : 0;
                ...
                <div className="flex items-start justify-between gap-4">
                      ...
                      {fTotal > 0 ? (
                        <div className="flex items-center gap-3 max-w-sm">
                          <div className="flex-1 h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full transition-all ${fTaux >= 75 ? 'bg-success-600' : fTaux >= 50 ? 'bg-warning-600' : 'bg-error-600'}`}
                              style={{ width: `${fTaux}%` }}
                            />
                          </div>
                          <span className={`text-xs font-medium flex-shrink-0 ${fTaux >= 75 ? 'text-success-600' : fTaux >= 50 ? 'text-warning-600' : 'text-error-600'}`}>
                            {fTaux}%
                          </span>
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <span className="flex items-center gap-1 text-xs text-success-600">
                              <div className="w-1.5 h-1.5 rounded-full bg-success-600" />{fPresent}
                            </span>
                            <span className="flex items-center gap-1 text-xs text-error-600">
                              <div className="w-1.5 h-1.5 rounded-full bg-error-600" />{fAbsent}
                            </span>
                            {fRetard > 0 && (
                              <span className="flex items-center gap-1 text-xs text-brand-600">
                                <div className="w-1.5 h-1.5 rounded-full bg-brand-600" />{fRetard}
                              </span>
                            )}
                          </div>
                        ...
                        <span className="text-xs text-subtext-color">Aucune session</span>
                      )}
                  ...
                  <div className="flex items-center gap-4 flex-shrink-0">
                    <div className="text-right hidden md:block">
                      <div className="text-xs text-subtext-color">Debut</div>
                      <div className="text-xs font-medium text-default-font tabular-nums">{f.dateDebut}</div>
                      <div className="text-xs text-subtext-color mt-1">Fin prevue</div>
...
<div className="bg-neutral-50 border border-neutral-200 rounded-lg p-6 mb-6">
  <div className="flex gap-6">
    ...
    <div className="flex-shrink-0 hidden lg:block">
      <div className="w-64 h-full rounded-lg overflow-hidden border border-neutral-200 bg-neutral-100">
        <img 
          src="https://res.cloudinary.com/dzd7tk6xn/image/upload/v1774620422/Group_38265_dkaf1y.png"
          alt={player.name}
        ...
        <div className="bg-neutral-50 border border-neutral-200 rounded-lg">
          ...
          {activeTab === 'infos'          ? <InformationsTab player={player} /> :
